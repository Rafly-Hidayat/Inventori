...
function pagination(param, res, table, column){
   ...
   pool.getConnection((err, connection) => {
       connection.query(`SELECT * FROM ${table}`, (err, rows) => {
           ...
           const query = `SELECT * FROM ${table} WHERE ${column[0]} OR ${column[1]} OR ${column[2]} OR ${column[3]} OR ${column[4]} OR ${column[5]} LIKE '%${search}%' ORDER BY ${sort} LIMIT ${limit} OFFSET ${offset}`
